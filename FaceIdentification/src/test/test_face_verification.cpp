...
void countDiffTime(long end, long start, const char * msg)
{
  std::cout<< msg << (end-start)/cv::getTickFrequency() << endl;
}
...
// Initialize face detection model
  ...
  countDiffTime(btime,atime,"Load Bin:");
  atime = btime;
 ...
 char* dstImg=argv[2];
  ...
  btime = cv::getTickCount();
  countDiffTime(btime,atime,"Convert2Gray:");
  atime = btime;
  ...
  btime = cv::getTickCount();
  countDiffTime(btime,atime,"Face Detect:");
  atime = btime;
  ...
  btime = cv::getTickCount();
  countDiffTime(btime,atime,"Mark Point:");
  atime = btime;
...
*/
  ...
  btime = cv::getTickCount();
  countDiffTime(btime,atime,"Extract Feature:");
  atime = btime;

  // Caculate similarity of two faces
  float sim = face_recognizer.CalcSimilarity(gallery_fea, probe_fea);
  std::cout <<"Similarity Value:"<< sim <<" size:"<<sizeof(probe_fea)<<endl;


  btime = cv::getTickCount();
  countDiffTime(btime,atime,"Calculate Similarity:");
  atime = btime;
